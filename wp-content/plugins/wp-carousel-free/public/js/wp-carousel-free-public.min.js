    ...
    jQuery("body").find(".wpcp-carousel-section.wpcp-standard").each((function() {
        ...
        if ("flip" == wpcpSwiperData.effect) {
            function fade_effect(slidesToShow) {
                for (var fade_items = $(`#${carousel_id} .swiper-wrapper >.single-item-fade`), i = 0; i < fade_items.length; i += slidesToShow) fade_items.slice(i, i + slidesToShow).wrapAll('<div class="swiper-slide"><div class="swiper-slide-kenburn"></div></div>');
                $(`#${carousel_id} .swiper-slide-kenburn`).each((function() {
                    var empty_items = slidesToShow - $(this).find(".single-item-fade").length;
                    if (empty_items > 0)
                        for (let i = 0; i < empty_items; i++) $(this).append(`<div class="single-item-fade" style="width:${100/slidesToShow}%;"></div>`)
                })), $(fade_items).css("width", `${100/slidesToShow}%`)
            }
            $(window).width() > wpcpSwiperData.responsive.desktop ? fade_effect(wpcpSwiperData.slidesToShow.lg_desktop) : $(window).width() > wpcpSwiperData.responsive.laptop ? fade_effect(wpcpSwiperData.slidesToShow.desktop) : $(window).width() > wpcpSwiperData.responsive.tablet ? fade_effect(wpcpSwiperData.slidesToShow.laptop) : $(window).width() > wpcpSwiperData.responsive.mobile ? fade_effect(wpcpSwiperData.slidesToShow.tablet) : $(window).width() > 0 && fade_effect(wpcpSwiperData.slidesToShow.mobile), wpcpSwiperData.slidesToShow.mobile = 1, wpcpSwiperData.slidesToShow.tablet = 1, wpcpSwiperData.slidesToShow.desktop = 1, wpcpSwiperData.slidesToShow.laptop = 1, wpcpSwiperData.slidesToShow.lg_desktop = 1
        }
